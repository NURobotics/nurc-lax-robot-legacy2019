...
namespace LGR {
  struct Ball {
    ...
    void calcFinalPos(double td, robot r);
    
    void print();
    void reset();
  };
  
  void Ball::calcFinalPos(double td, robot r) {
    ...
    if (consecFramesFound == 2) {
      ...
      double dt = (Z.pf - Z.p1)/Z.vel;

      X.pf = (X.vel * dt) * 0.5 + X.pf * 0.5;
      Y.pf = (0.5 * GRAVITY * (dt*dt) + Y.vel * dt + Y.p1)  * 0.5 + Y.pf * 0.5;
    }
