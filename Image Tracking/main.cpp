...
void GetCooridnates(VideoCapture capture, double* x, double* y, Mat* cameraFeed, Mat* HSV, Mat* threshold, bool* objectFound, bool showCrossHairs, bool LeftCam) {
    ...
    if (LeftCam) *cameraFeed = *cameraFeed*5;
...
int main(int argc, char* argv[]) {
    ...
    // Distance between each camera and the cameras and the ground
    double dxCameras = 0.362;
    double dyGround = 1;

    Mat K(3,3,CV_64F, Scalar(0.0));
    K.at<double>(0,0) = 995.017352724241164;
    K.at<double>(1,1) = 996.788504391169340;
    K.at<double>(0,2) = 639.754371102307573;
    K.at<double>(1,2) = 371.061867974806205;
    K.at<double>(2,2) = 1;

    Mat distortion_coeffs(5,1,CV_64F);
    distortion_coeffs.at<double>(0,0) = -0.392080477762577;
    distortion_coeffs.at<double>(1,0) = 0.144522160514508;
    distortion_coeffs.at<double>(2,0) = -0.003338987421132;
    distortion_coeffs.at<double>(3,0) = -0.000291551741632;
    distortion_coeffs.at<double>(4,0) = 0.000000000000000;
    ...
    Mat projL = K*transformL;
    Mat projR = K*transformR;
	...
	while(1) {
        ...
        /*if (objectFoundL)
            cout << "X1: " << xc1 << " / Y1: " << yc1 << endl;
        if (objectFoundR)
            cout << "X2: " << xc2 << " / Y2: " << yc2 << endl;
            */
        long long start = PerformanceCounter();
        if (objectFoundL && objectFoundR) {
            ...
            undistortPoints(ballCoordL,undistoredCoordL,K,distortion_coeffs);
            undistortPoints(ballCoordR,undistoredCoordR,K,distortion_coeffs);
